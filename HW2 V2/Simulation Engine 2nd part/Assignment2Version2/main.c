...
#define G 5					// time required on the ground before departing
#define MAX_ARRIVALS 5          // maximum number of aircraft arrivals
...
#define A 2                     // mean of exponential distribution
...
typedef enum {ARRIVAL, LANDED, DEPARTED, DISAPPEARED} EventKind;
typedef enum {FREE, BUSY} RunwayState;

// Statistics variables
double  STAT_totalWaitingTime   = 0;                // total waiting time over all aircrafts
double  STAT_totalWaitingTimetoDepart   = 0;        // total waiting time over all aircrafts to depart
...
void callback(void* data) {
    ...
    else if (eventData->eventKind == DEPARTED) {
        departed(eventData);
    }
...
void arrival (EventData *arrivalData) {
    ...
    // check if the runway is free, and schedule landing
    if (runwayState == FREE) {
...
void landed (EventData *landedData) {
    ...
    // schedule current aircraft to depart after G seconds
    double ts = current_time() + G;                 // time at which aircraft should "depart"
    
    EventData *newDeparted = (EventData *) malloc(sizeof(EventData));
    if (newDeparted == NULL) FatalError("landed", "Could not allocate memory for disappeared event.");
    
    newDeparted->eventKind = DEPARTED;
    newDeparted->eventParam.departedEvent.aircraft = landedData->eventParam.landedEvent.aircraft;
    newDeparted->eventParam.departedEvent.aircraft->waittoDepart = ts;
    
    schedule(ts, newDeparted);
    ...
    if (nWaiting > 0) {
        ...
        schedule(current_time(), newLanded);
    }
...
/*
 * Event handler for a departed aircraft
 *
 * @param landedData the event data
 */

void departed (EventData *departedData) {

    if(runwayState == BUSY){
        nWaitDepart += 1;
        Aircraft* aircraft = departedData->eventParam.departedEvent.aircraft;
        fifo_push(toDepartQ, aircraft);
        printf("Aircraft %d in Q.\n", aircraft->aircraftID);
        return;
    }
    
#ifdef VERBOSE
    printf("TS = %f: Aircraft %d departed.\n", current_time(), departedData->eventParam.departedEvent.aircraft->aircraftID);
    fflush(stdout);
#endif
    ...
    double ts = current_time() + R;                 // time at which aircraft should "disappear"
    ...
    newDisappeared->eventParam.disappearedEvent.aircraft = departedData->eventParam.departedEvent.aircraft;
    newDisappeared->eventParam.departedEvent.aircraft->endwaittoDepart = ts;
...
void disappeared(EventData *disappearedData) {
    ...
    if (nWaitDepart>0){
        ...
        nWaitDepart -= 1;
        Aircraft* aircraft = fifo_pop(toDepartQ);
        
        EventData *newDeparted = (EventData *) malloc(sizeof(EventData));
        if (newDeparted == NULL) FatalError("landed", "Could not allocate memory for disappeared event.");
        
        newDeparted->eventKind = DEPARTED;
        newDeparted->eventParam.departedEvent.aircraft = aircraft;
        
        schedule(current_time() + R, newDeparted);
    ...
    STAT_totalWaitingTimetoDepart += aircraft->endwaittoDepart - aircraft->waittoDepart;
...
int main(int argc, const char * argv[]) {
    ...
    printf("Average waiting time for landing: %.2f\n", STAT_totalWaitingTime / STAT_numAircrafts);
    printf("Average waiting time for departure: %.2f\n", STAT_totalWaitingTimetoDepart / STAT_numAircrafts);
